...
# Uses bash for FUNCNAME otherwise optimized for shell (FIXME)
[ "$0" = sh ] && eerror "We are using FUNCNAME which blocks POSIX compatibility"
...
die() {
	...
	# Shellcompat
	# INFO: Use MYFUNCNAME instead of FUNCNAME in this script refering to name of the function
	# shellcheck disable=SC2128 # Exporting MYFUNCNAME for compatibility
	[ -z "${FUNCNAME}" ] && MYFUNCNAME="die"

	# We need to use MYFUNCNAME in each function for it to work (expecting to output name of function from which die was called)
	fixme "Implement FUNCNAME[1] for POSIX sh"

	# Output error if more then expected arguments is parsed
	[ -n "$syntax_err" ] && die 2 "Unexpected argument has been parsed in die - '$syntax_err'"
